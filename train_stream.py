...
def dataset_collate(batch):
    ...
    now_labels = []
    future_labels = []
    for l_img,n_img,n_label,f_label in batch:
        last_images.append(l_img)
        now_images.append(n_img)
        now_labels.append(n_label)
        future_labels.append(f_label)
    last_images = torch.stack(last_images,dim=0).type(torch.FloatTensor)
    now_images = torch.stack(now_images,dim=0).type(torch.FloatTensor)
    now_labels = [torch.tensor(ann).type(torch.FloatTensor) for ann in now_labels]
    future_labels = [torch.tensor(ann).type(torch.FloatTensor) for ann in future_labels]
    return last_images, now_images, now_labels, future_labels

def fit_one_epoch(model_train, model, ema, stream_yolo_loss, loss_history, optimizer, epoch, epoch_step, gen, Epoch, cuda, fp16, scaler, save_period, save_dir, local_rank=0):
    loss = 0
    #val_loss = 0
    ...
    for iteration, batch in enumerate(gen):
        ...
        images_last, images_now, targets_now, targets_future = batch
        if cuda:
            images_last = images_last.cuda()
            images_now = images_now.cuda()
            targets_now = [ann.cuda() for ann in targets_now]
            targets_future = [ann.cuda() for ann in targets_future]
        # ----------------------#
        #   清零梯度
        # ----------------------#
        optimizer.zero_grad()
        if not fp16:
            ...
            #   前向传播
            # ----------------------#
            outputs = model_train(images_last,images_now)

            # ----------------------#
            #   计算损失
            # ----------------------#
            loss_value = stream_yolo_loss(outputs, targets_now, targets_future)
...
if __name__ == "__main__":
    ...
    # ---------------------------------------------------------------------#
    classes_path = 'model_data/argo_classes.txt'
    # ----------------------------------------------------------------------------------------------------------------------------#
    ...
    stream_yolo_loss = StreamYOLOLoss(num_classes,fp16)
    if local_rank == 0:
    ...
    if True:
        ...
        #   判断每一个世代的长度
        # ---------------------------------------#
        train_dataset = StreamDataSet(make_triplet_sets())
        ...
        gen = DataLoader(train_dataset, shuffle=shuffle, batch_size=batch_size, num_workers=0,collate_fn=dataset_collate)
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, ema, stream_yolo_loss, loss_history, optimizer, epoch, epoch_step, gen, UnFreeze_Epoch, Cuda, fp16, scaler, save_period, save_dir,
                          local_rank)
