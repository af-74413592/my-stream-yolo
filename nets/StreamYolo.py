...
from nets.yolo import YOLOPAFPN
...
class StreamYolo(nn.Module):
    def __init__(self, num_classes):
        ...
        depth, width = 1.00, 1.00
        self.backbone = YOLOPAFPN(depth, width, depthwise=False)
        self.DFPNet = DFPNet()
        self.head = YOLOXHead(num_classes, width, depthwise=False)

    def forward(self,lastimg,nowimg):
        last_inputs = self.backbone(lastimg)
        now_inputs = self.backbone(nowimg)
        last_dfp = self.DFPNet(last_inputs)
        now_dfp = self.DFPNet(now_inputs)
        dfp_outs = []
        for i in range(3):
            self.dynamic = torch.cat([last_dfp[i],now_dfp[i]],dim=1)
            self.static = now_inputs[i]
            dfp_outs.append(self.dynamic+self.static)
        return self.head(dfp_outs)


class DFPNet(nn.Module):
    def __init__(self,in_channels = [256, 512, 1024], act = "silu"):
        super(DFPNet, self).__init__()
        self.dfps = nn.ModuleList()
        for i in range(len(in_channels)):
            self.dfps.append(
                BaseConv(in_channels=int(in_channels[i]), out_channels=int(in_channels[i])//2, ksize=1, stride=1,
                         act=act))
...
class TrendAwareLoss(nn.Module):
    def __init__(self,t=0.3,v=1.4):
        super(TrendAwareLoss, self).__init__()
        self.t = t
        self.v = v

    def forward(self,now_boxes,future_box):
        # futurebox_area = (future_box[2] - future_box[0]) * (future_box[3] - future_box[1])
        # nowboxes_area = (now_boxes[:, 2] - now_boxes[:, 0]) * (now_boxes[:, 3] - now_boxes[:, 1])
        # xx1 = torch.maximum(future_box[0], now_boxes[:, 0])
        # yy1 = torch.maximum(future_box[1], now_boxes[:, 1])
        # xx2 = torch.minimum(future_box[2], now_boxes[:, 2])
        # yy2 = torch.minimum(future_box[3], now_boxes[:, 3])
        #
        # w, h = torch.maximum(torch.Tensor([0]), xx2 - xx1), torch.maximum(torch.Tensor([0]), yy2 - yy1)
        #
        # ovr_area = w * h
        #
        # ious = ovr_area / (futurebox_area + nowboxes_area - ovr_area+ 1e-16)
        tl = torch.max(
            (now_boxes[:, :2] - now_boxes[:, 2:] / 2), (future_box[:2] - future_box[2:] / 2)
        )
        br = torch.min(
            (now_boxes[:, :2] + now_boxes[:, 2:] / 2), (future_box[:2] + future_box[2:] / 2)
        )

        area_n = torch.prod(now_boxes[:, 2:], 1)
        area_f = future_box[2] * future_box[3]

        en = (tl < br).type(tl.type()).prod(dim=1)
        area_i = torch.prod(br - tl, 1) * en
        area_u = area_n + area_f - area_i
        iou = (area_i) / (area_u + 1e-16)
        mIou = torch.max(iou)
        if mIou >= self.t:
            wi = 1/mIou
        else:
            wi = torch.Tensor([1/self.v]).type_as(mIou).squeeze(0)
        return wi


class StreamIOUloss(nn.Module):
    def __init__(self):
        super(StreamIOUloss, self).__init__()
...
class StreamYOLOLoss(nn.Module):
    def __init__(self, num_classes, fp16, strides=[8, 16, 32]):
        super().__init__()
        self.num_classes = num_classes
        self.strides = strides
        self.trend_aware_loss = TrendAwareLoss()
        self.bcewithlog_loss = nn.BCEWithLogitsLoss(reduction="none")
        self.iou_loss = StreamIOUloss()
        self.grids = [torch.zeros(1)] * len(strides)
        self.fp16 = fp16

    def forward(self, inputs, labels_now, labels_future):
        # self.now_boxes = labels_now[...,:4]
        # self.num_classes = labels_now[...,4]
        # self.future_boxes = labels_future[...,:4]
        # self.future_classes = labels_future[...,4]
        ...
        return self.get_losses(x_shifts, y_shifts, expanded_strides, labels_now, labels_future, torch.cat(outputs, 1))
    ...
    def get_losses(self, x_shifts, y_shifts, expanded_strides, labels_now, labels_future, outputs):
        # -----------------------------------------------#
        ...
        for batch_idx in range(outputs.shape[0]):
            num_gt = len(labels_future[batch_idx])
            if num_gt == 0:
                cls_target = outputs.new_zeros((0, self.num_classes))
                reg_target = outputs.new_zeros((0, 4))
                w_matched = outputs.new_zeros((0, 1))
                ...
                # -----------------------------------------------#
                lt_bboxes_per_image = labels_now[batch_idx][..., :4].type_as(outputs)
                lt_classes = labels_now[batch_idx][..., 4].type_as(outputs)
                gt_bboxes_per_image = labels_future[batch_idx][..., :4].type_as(outputs)
                gt_classes = labels_future[batch_idx][..., 4].type_as(outputs)
                bboxes_preds_per_image = bbox_preds[batch_idx]
                cls_preds_per_image = cls_preds[batch_idx]
                obj_preds_per_image = obj_preds[batch_idx]

                gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg_img, w_per_img = self.get_assignments(
                    num_gt, total_num_anchors, lt_bboxes_per_image,lt_classes,gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
                ...
                w_per_img = torch.stack(w_per_img,dim=0)
                w_matched = w_per_img[matched_gt_inds]
            cls_targets.append(cls_target)
            reg_targets.append(reg_target)
            w_matchedes.append(w_matched)
        ...
        num_fg = max(num_fg, 1)
        loss_iou = (torch.cat(w_matchedes,dim=0) * self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets)).sum()
    ...
    @torch.no_grad()
    def get_assignments(self, num_gt, total_num_anchors,lt_bboxes_per_image,lt_classes, gt_bboxes_per_image, gt_classes, bboxes_preds_per_image,
                        cls_preds_per_image, obj_preds_per_image, expanded_strides, x_shifts, y_shifts):
        w_per_image = []
        for gt_cls,gt_box in zip(gt_classes,gt_bboxes_per_image):
            new_now_boxes = []
            for i,lt_cls in enumerate(lt_classes):
                if gt_cls == lt_cls:
                    new_now_boxes.append(lt_bboxes_per_image[i])
            if len(new_now_boxes) != 0:
                wi = self.trend_aware_loss(torch.stack(new_now_boxes,dim=0),gt_box)
                w_per_image.append(wi)
            else:
                wi = torch.Tensor([1 / 1.4]).to('cuda' if torch.cuda.is_available() else 'cpu').squeeze(0)
                w_per_image.append(wi)
        ...
        return gt_matched_classes, fg_mask, pred_ious_this_matching, matched_gt_inds, num_fg, w_per_image
